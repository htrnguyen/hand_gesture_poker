...
export class TarotScene {
    ...
    createCards(cardDataList) {
        if (!cardDataList || cardDataList.length === 0) {
            console.error('[SCENE] No card data provided!');
            return;
        }
        ...
        const createRoundedCardGeometry = (width, height, radius) => {
            ...
            for (let i = 0; i < positionAttribute.count; i++) {
                const xPos = positionAttribute.getX(i);
                const yPos = positionAttribute.getY(i);
                
                const u = (xPos + width / 2) / width;
                const v = (yPos + height / 2) / height;
        ...
        const geometry = createRoundedCardGeometry(CARD_CONFIG.cardWidth, CARD_CONFIG.cardHeight, 0.05);

        CARD_CONFIG.tiers.forEach((tier, tierIndex) => {
            for (let i = 0; i < tier.count; i++) {
                if (cardCount >= cardDataList.length) break;

                const data = cardDataList[cardCount];
                if (!data) continue;
                ...
                if (data.imageUrl) {
                    textureLoader.load(
                        ...
                        undefined,
                        (err) => console.warn(`Failed to load texture: ${data.imageUrl}`)
                    );
                }
